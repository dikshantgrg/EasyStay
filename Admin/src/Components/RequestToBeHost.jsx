...
const RequestToBeHost = () => {
  ...
  const fetchUsers = async () => {
    try {
      setLoading(true);
      const response = await axios.get("http://localhost:8000/users/host-approval");
  ...
  const handleStatusChange = async (e, userId) => {
    ...
    try {
      const response = await axios.put(`http://localhost:8000/host-approval`, {
        userId,
        action: newStatus === "approved" ? "approve" : "reject",
      });

      // Show success message
      toast.success(`User status updated to "${newStatus}" successfully!`); // Toast for success
      ...
      toast.error(errorMessage); // Toast for update error
    }
  };
